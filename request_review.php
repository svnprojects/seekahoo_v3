...
<?php 
    ...
    $str = $_POST['search_type'];
	$exp = explode(",",$str);
	
	//Sign up
	$url = $api_url.'users/pro_sign_up'; 
	$fields = array(
	'is_social_media_user'=>2,
	'user_type'=>4,
	'tag_name'=>$_POST['First_name'].$_POST['Last_name'],
	'name'=>$_POST['First_name'].' '.$_POST['Last_name'],
	'description'=>$_POST['user_description'],
	'email'=>$_POST['email'],
	'password'=>base64_encode('xvalue'),
	...
	$user = $dat->data->user_id;
	
	
	$user_id = $user;
	$media = preg_replace("/[^a-zA-Z0-9.]+/","", $_FILES['file']["name"]);
	//$media = $_FILES['file']["name"];
	if($user_id !='' && file_exists($file_path."uploads/".$user_id))
						{
						
	move_uploaded_file($_FILES['file']["tmp_name"], 'import/images/'.$media);
    $a = copy($main_path.'import/images/'.$media, $file_path.'uploads/'.$user_id.'/post/photos/'.$media);
	$src = $file_path.'uploads/'.$user_id.'/post/photos/'.$media;
	$dest = $file_path.'uploads/'.$user_id.'/post/thumbs/'.$media;
	make_thumb($src,$dest);
	$media_org_url ='uploads/'.$user_id.'/post/photos/'.$media;
	$media_thumb_url ='uploads/'.$user_id.'/post/thumbs/'.$media;
						}
						else
						{
						
		$oldmask = umask(0);
		mkdir($file_path."uploads/".$user_id,0777,TRUE);
		mkdir($file_path."uploads/".$user_id."/post",0777,TRUE);
		mkdir($file_path."uploads/".$user_id."/profile",0777,TRUE);
		mkdir($file_path."uploads/".$user_id."/post/photos",0777,TRUE);
		mkdir($file_path."uploads/".$user_id."/post/thumbs",0777,TRUE);
		mkdir($file_path."uploads/".$user_id."/profile/photos",0777,TRUE);
		mkdir($file_path."uploads/".$user_id."/profile/thumbs",0777,TRUE);
	...
	//Sign up
	$url = $api_url.'users/add_media_web'; 
	$fields = array(
	'media_org_url'=>$media_org_url,
	...
	//execute post
	$result = curl_exec($ch);
	$dat2 = json_decode($result);
	$media_id = $dat2->data->media_id;
	
	//exit;
	//Add Post
	$url = $api_url.'posts/add_post_3'; 
	$hash = explode("##",$exp[0]);
	$data_hash = array();
	foreach($hash as $hashing)
	{
		$data_hash[] = array("hash_tag" => '#'.$hashing);
	}
	$dat = array("hash_tags" =>$data_hash);
	$hash_tags = json_encode($dat);
	$fields = array(
	'user_id'=>$user,
	'media_id'=>$media_id,
	'hash_tags'=>$hash_tags,
	'post_type'=>$_POST['tag_id'],
	...
	$hash = $dat1->data->hash_id;

	//Get pro by category
	$url = $api_url.'posts/get_pro_by_category'; 
	$fields = array(
	'user_id'=>$user,
	'distance'=>25,
	'category_id'=>$exp[1]
